...
fn find_vulnerabilities(document: &Html) -> HashMap<String, i32> {
    ...
    for form in document.select(&form_selector) {
        if let Some(method) = form.value().attr("method") {
            if method.to_lowercase() == "post" {
                let counter = result.entry("INJECTION".to_string()).or_insert(0);
                *counter += 1;
            }
        }
    }
...
fn scrape(url: &str) -> String {
    ...
    while let Some(curr_url) = stack.pop() {
        ...
        pages.insert(curr_url.clone(), find_vulnerabilities(&document));
